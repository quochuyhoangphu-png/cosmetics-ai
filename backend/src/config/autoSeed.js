 ...
 * Giúp tránh việc phải chạy lệnh bằng Render Shell (yêu cầu trả phí)
 */
...
async function autoSeed() {
  try {
    ...
    if (productCount === 0) {
      console.log('🌱 Database has no products. Auto-seeding 9 commercial products...');
      await Product.bulkCreate(initialProducts);
      console.log('✅ Commercial products successfully seeded.');
    }

    if (ingredientCount === 0) {
      console.log('🌱 Database has no ingredients. Auto-seeding 8 active ingredients...');
      await Ingredient.bulkCreate(initialIngredients);
      console.log('✅ Active ingredients successfully seeded.');
    }
  } catch (error) {
    console.error('❌ Error running auto-seed on startup:', error.message);
  }
}
